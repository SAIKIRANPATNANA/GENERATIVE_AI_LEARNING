...
def set_bg_hack_url():
    ...
    st.markdown(
         ...
         <style>
         .stApp {{
             background: url("https://4kwallpapers.com/images/wallpapers/macos-monterey-stock-black-dark-mode-layers-5k-2560x1080-5889.jpg");
...
rainbow_title_html = """<p class="rainbow-title-text">Questions And Answers</p>"""
st.markdown(rainbow_title_html, unsafe_allow_html=True)
st.markdown("""<h5 style='font-size:25px;text-align:center;color:#39f213;'><i>Developed By Sai Kiran  For Project Saadhana</i></h5>""", unsafe_allow_html=True)
...
if st.session_state.uploaded_file is not None:
    ...
    st.session_state.number = st.slider('Pick the number of questions?', 0, 100, 25)
    ...
    if choice == ":rainbow[Yes]":
        with st.expander("Scheme of Evaluation"):
            st.write(f'- You will be asked {str(st.session_state.number)} Descriptive Questions.')
            st.write('- Each question carries 2 ⭐')
