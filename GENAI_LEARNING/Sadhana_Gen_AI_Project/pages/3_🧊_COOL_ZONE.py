...
@st.cache_data
def get_mcq_general_cacher(text,difficulty_level,number,random):
    try: 
        output1,output2 = get_mcq_general(text,difficulty_level,number)
...
@st.cache_data
def get_mcq_specific_cacher(topic_name,difficulty_level,number,random):
    try:
        output1,output2 = get_mcq_specific(topic_name,difficulty_level,number)
        if(output1!=0 and output2!=0):
            cleaned_questions,cleaned_answers = output1,output2
            return cleaned_questions,cleaned_answers
        else:
...
@st.cache_data
def get_qa_general_cacher(text,difficulty_level,number,random):
    try:
        output1,output2 = get_qa_general(text,difficulty_level,number)
        if(output1!=0 and output2!=0):
            cleaned_questions,cleaned_answers = output1,output2
            return cleaned_questions,cleaned_answers
        else:
...
@st.cache_data
def get_qa_specific_cacher(topic_name,difficulty_level,number,random):
    try:
        output1,output2 = get_qa_specific(topic_name,difficulty_level,number)
        if(output1!=0 and output2!=0):
            cleaned_questions,cleaned_answers = output1,output2
            return cleaned_questions,cleaned_answers
        else:
...
if st.session_state.uploaded_file is not None and st.session_state.topic_name is not None and st.session_state.difficulty_level is not None and st.session_state.text is not None and st.session_state.number is not None:
    
    if 'user_answers' not in st.session_state or st.session_state.user_answers is not None:
        if st.session_state.mode == 'objective':
            st.session_state.user_answers = [None]*st.session_state.number
    ...
    with st.status('Preparing Questions based on your preference..!'):
        ...
        if st.session_state.mode == 'objective' and st.session_state.flag:
            if(st.session_state.topic_name=='overall'):
                output1,output2  = get_mcq_general_cacher(st.session_state.text,st.session_state.difficulty_level,st.session_state.number,st.session_state.random)
                if(output1!=0 and output2!=0):
                ...
                output1,output2 =  get_mcq_specific_cacher(st.session_state.topic_name,st.session_state.difficulty_level,st.session_state.number,st.session_state.random)
                if(output1!=0 and output2!=0):
                    st.session_state.cleaned_questions,st.session_state.cleaned_answers =  output1,output2
                else:
                    st.warning('Sorry, Something went wrong..!',icon="⚠️")
            ...
            if(st.session_state.topic_name=='overall'):
                output1,output2 = get_qa_general_cacher(st.session_state.text,st.session_state.difficulty_level,st.session_state.number,st.session_state.random)
                if(output1!=0 and output2!=0):
                    st.session_state.cleaned_questions,st.session_state.cleaned_answers = output1,output2
                else:
                    st.warning('Sorry, Something went wrong..!',icon="⚠️")
                ...
                output1,output2 = get_qa_specific_cacher(st.session_state.topic_name,st.session_state.difficulty_level,st.session_state.number,st.session_state.random)
                if(output1!=0 and output2!=0):
                    st.session_state.cleaned_questions,st.session_state.cleaned_answers = output1,output2
                else:
                    st.warning('Sorry, Something went wrong..!',icon="⚠️")
